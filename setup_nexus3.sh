...
# add_nexus_proxy_repo [repo-id] [repo-url] [nexus-username] [nexus-password] [nexus-url]
#
function add_nexus_npmproxy_repo() {
  local _REPO_ID=$1
  local _REPO_URL=$2
  local _NEXUS_USER=$3
  local _NEXUS_PWD=$4
...
# add_nexus_release_repo [repo-id] [nexus-username] [nexus-password] [nexus-url]
#
function add_nexus_release_repo() {
  local _REPO_ID=$1
  local _NEXUS_USER=$2
  local _NEXUS_PWD=$3
  local _NEXUS_URL=$4
...
#
# add_nexus_group_proxy_repo [comma-separated-repo-ids] [group-id] [nexus-username] [nexus-password] [nexus-url]
#
function add_nexus_group_proxy_repo() {
  local _REPO_IDS=$1
  local _GROUP_ID=$2
  local _NEXUS_USER=$3
  local _NEXUS_PWD=$4
...
# add_nexus_docker_repo [repo-id] [repo-port] [nexus-username] [nexus-password] [nexus-url]
#
function add_nexus_docker_repo() {
  local _REPO_ID=$1
  local _REPO_PORT=$2
  local _NEXUS_USER=$3
  local _NEXUS_PWD=$4
...
# Red Hat Proxy Repos
add_nexus_proxy_repo redhat-ga https://maven.repository.redhat.com/ga/ $1 $2 $3
#add_nexus_proxy_repo jboss https://repository.jboss.org/nexus/content/groups/public/ $1 $2 $3

# Repo Group to include all proxy repos
#add_nexus_group_proxy_repo redhat-ga,jboss,maven-central,maven-releases,maven-snapshots maven-all-public $1 $2 $3
add_nexus_group_proxy_repo redhat-ga,maven-central,maven-releases,maven-snapshots maven-all-public $1 $2 $3

# NPM Proxy Repo
add_nexus_npmproxy_repo npm https://registry.npmjs.org/ $1 $2 $3
# Private Docker Registry
add_nexus_docker_repo docker 5000 $1 $2 $3

# Maven release Repo
add_nexus_release_repo releases $1 $2 $3
